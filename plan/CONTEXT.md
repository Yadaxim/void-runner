...
- **2026-05-09** — **Phase 4.5 Session D shipped:** added **`WorldFile.species`** / **`Species`** model and faction extensions (**`type`**, **`speciesComposition`**, **`homeLandableId`**, **`bubbleStance`**). **`techArchetype`** lives on **species** only (`TechArchetype` enum). **`validateWorldFile`** checks species IDs, archetypes, tech archetypes, habitats, faction composition, bubble stance, and home-landable rules.  

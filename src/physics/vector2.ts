...
export class Vector2 implements Vector2Like {
  ...
  angleTo(other: Vector2Like): number { return Math.atan2(other.y - this.y, other.x - this.x); }
  ...
  static fromAngle(angle: number): Vector2 { return new Vector2(Math.cos(angle), Math.sin(angle)); }
